...
0 For gas, fluids, and liquids at high T only the **total entropy** is well defined.

## Configurational entropy
:::{.columns}
::: {.column width="50%"}
At low temperatures, in the glassy state, we can separate the total entropy into a vibrational and configurational part and define
$$ S_{conf}(T) = S(T) -S_{vib}(T) +$$

For several fluids, this *decreasess* with temperature in an intriguing way.
...
![Kauzmann paradox](./figs/kauzmann.png)
:::
:::
...
$$
with $T_0 \approx T_K$. This connects the kinetic slowdown to loss of configurational entropy.
...
:::{.columns}
::: {.column width="70%"}
**Dynamical facilitation theory** provides an alternative to thermodynamic models:
...
Unlike VFT, no finite-temperature divergence, but still captures super-Arrhenius behavior.
:::
::: {.column width="30%"}
...
## Demonstration of facilitated dynamics
